...
## Security warning
When logging into i2b2 services by means other than the patched web client, the PM module will authenticate the user with i2b2's built-in authentication mechanism, that is, by checking the username and password against what is stored in the `PM_USER_DATA` table in the project management cell's schema. For that reason, we strongly recommend that you populate the password field in `PM_USER_DATA` with a random password. While leaving the password field set to NULL currently results in the user not being able to login to i2b2 through the built-in mechanism, we are unaware of any guarantee that the behavior will stay that way. 

If your users need to login to i2b2 by means other than the patched web client (for example, the admin client or the workstation client), the `PM_USER_DATA` table needs to contain a password that is known to the user.
